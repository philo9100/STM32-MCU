 ...
 * 
 * 基本定时器计数器时钟频率 (f(tim)) 计算如下
 * f(tim) = f(apb1) / apb1_prescaler(APB1预分频器) 
 * 
 * 当APB1预分频器大于1时，定时器时钟会被再乘以2，总之，TIM6（基本定时器）的时钟频率为 72MHz
 * 
 * 计数器的周期由定时器的预分频器（PSC, Prescaler）和自动重装载寄存器（ARR, Auto-reload Register）的值决定。
 * 
 * T = (ARR + 1) / f(tim) / (PSC + 1)
 * 
...
void Driver_TIM6_Init(void)
{
    ...
    */
    ...
    /* 为了避免一启动不到1s就立即进入更新中断，可以先产生一个更新事件，将计时器的预分频器的预分频值写入到影子寄存器 */
    TIM6->EGR |= TIM_EGR_UG;    // 这个执行过后会先将计时器的预分频值写于影子寄存器，按照自定义的预分频值的频率进行计数
                                // 否则计数器开始的时候会按照72MHz的频率计数10000次，不到1s产生一次更新中断后才将预分频值写入影子寄存器
    
    /* 由于手动产生了一次更新中断 SR寄存器的 UIF 中断挂起标志位会会置为1，这里还没有使能更新中断执行中断服务函数用来清除该位，因此要手动清除 */
    TIM6->SR &= ~TIM_SR_UIF;
