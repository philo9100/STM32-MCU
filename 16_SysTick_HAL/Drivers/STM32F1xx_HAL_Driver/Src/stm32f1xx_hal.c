...
{
  /* 如果SysTick Timer的频率配置为1KHz，那么每经过1m */
  return uwTick;
}
...
{
  /*
  HAL_GetTick()函数返回 uwTick 需要中断来更新，tickstart 表示当前开始用来延时的 ms 数 
  
  例如，现在已经产生 100 个中断（即经过100 ms），tickstart 的值为 100
  */
  ...
  */
  ...
  /* 每经过 1ms (HAL_GetTick() - tickstart) 的值就会加 1
  例如，经过 3ms 后 (HAL_GetTick() - tickstart) 的值变为 3，此时依然小于 wait 的值
  会再进行一次while才会结束循环，恰好是要延时的时间
  */
