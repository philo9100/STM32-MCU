...
void HAL_GPIO_EXTI_IRQHandler(uint16_t GPIO_Pin)
{
  ...
  if (__HAL_GPIO_EXTI_GET_IT(GPIO_Pin) != 0x00u)  // 这个判断会将传入的GPIO_Pin 与 EXTI->PR 外部中断挂起寄存器进行与操作，在引脚产生外部中断时，会自动将PR寄存器对应位置为1，判断结果不为0，则说明该Pin确实产生了外部中断
  {
    __HAL_GPIO_EXTI_CLEAR_IT(GPIO_Pin); // 先将EXTI->PR 外部中断挂起寄存器对应的Pin产生中断的位置为1，清除中断挂起位，在执行对应Pin的外部中断服务函数
    /* 这是一个 _weak 函数，需要我们自己重写中断处理程序的逻辑 */
