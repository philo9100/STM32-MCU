...
void Driver_USART1_Init (void) {
    ...
    /* 3.6 使能串口的各种中断 */
    /* 在手册27.4节中可以看到各种串口中断，以接受数据为例配置如下两个中断 */
    USART1->CR1 |= USART_CR1_RXNEIE; // 使能接收非空中断
    USART1->CR1 |= USART_CR1_IDLEIE; // 使能空闲中断

    /*
    RXNE：读取数据寄存器不为空
    当RDR移位寄存器的内容已传输到USART_DR寄存器（指读取数据寄存器RDR）时，SR寄存器中（RXNE: Read data register not empty）位由硬件设置为1。
    如果USART_CR1寄存器中的RXNEIE=1，则生成中断。
    RXNE位通过读取USART_DR寄存器来清除。
    RXNE标志也可以通过向其写入零来清除。仅建议将该清除序列用于多缓冲器通信。    
    
    IDLE：检测到空闲线路。
    当检测到空闲线时，SR寄存器中（IDLE）位由硬件设置位1。
    如果USART_CR1寄存器中的IDLEIE=1，则生成中断。
    SR寄存器中（IDLE）位由软件序列清除（读取USART_SR寄存器，然后读取USART_DR寄存器（RDR））。
    注意：在RXNE位自身设置之前（即出现新的空闲线路），不会重复设置空闲位。

    RXNE 为 1，说明RXNE位没有通过读取USART_DR寄存器来清除。
    或者RXNE标志也没有通过向其写入零来清除。在RXNE位自身设置之前，不会重复设置空闲位。
    */
...
/**
 * @brief 串口1中断处理函数，因为开启了串口1的接受数据中断和检测空闲中断
 * 一旦接收到数据或者检测到空闲帧，就会触发中断，不用再轮询等待
 * @param  
 */
