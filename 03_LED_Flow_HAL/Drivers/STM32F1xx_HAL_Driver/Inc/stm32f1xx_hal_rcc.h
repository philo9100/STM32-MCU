...
#define __HAL_RCC_GPIOA_CLK_ENABLE()   do { \
                                        ...
                                        /* 将 RCC->APB2ENR 寄存器，既是对应的32位内存空间中的第三位赋值1，使用按位或操作 */\
                                        /* Delay after an RCC peripheral clock enabling */\
                                        tmpreg = READ_BIT(RCC->APB2ENR, RCC_APB2ENR_IOPAEN);\
                                        /* 用于指示变量tmpreg在其后的代码中没有被使用，通常用于消除编译器关于未使用变量的警告
                                           在这个特定的情况下，tmpreg被用于读取RCC->APB2ENR寄存器的值，这会引入一个微小的延迟，这是在启用RCC外设时钟后所需要的。
                                           然后，tmpreg的值并没有被使用，因此使用UNUSED(tmpreg)来消除可能的编译器警告。
                                         */\
                                        UNUSED(tmpreg); \
                                      } while(0U)
